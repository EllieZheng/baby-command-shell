...
UserInput::UserInput(): originalInput(std::string()), 
...
UserInput::UserInput(std::string & input): originalInput(input), 
    args(NULL), num_arg(0), redirct(NULL){
...
UserInput::UserInput(const UserInput & rhs): originalInput(rhs.originalInput),  
    args(NULL), num_arg(rhs.num_arg), redirct(new std::string[3]()){
  *this = rhs;
}

UserInput & UserInput::operator= (const UserInput & rhs){
  if (this != &rhs){
...
//take care of "\ "
std::size_t UserInput::findEnd(std::size_t begin, std::string & input, 
...
void UserInput::findRedirct(std::vector<std::string> & argsVec){
...
void UserInput::redirctOp(std::vector<std::string> & argsVec, 
